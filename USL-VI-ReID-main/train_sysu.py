...
def main_worker_stage1(args,log_s1_name):
    ...
    #Load Checkpoint
    #state_dict = torch.load('/home/shaorui/USL-VI-ReID-main/data/logs/sysu_s2/checkpoint.pth.tar')
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            cluster_loader_part_rgb = get_test_loader(dataset_part_rgb, args.height, args.width,256,
                                                       args.workers, 
                                                       testset=sorted(dataset_part_rgb.train))
            features_rgb, labels_rgb = extract_features(model, cluster_loader_rgb, print_freq=50,mode=1)#这里提取特征的网络是没有梯度的
            del cluster_loader_rgb
            features_rgb = torch.cat([features_rgb[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)#这里的f为file_path相当于fname
            labels_rgb = [labels_rgb[f].item() for f, _, _ in sorted(dataset_rgb.train)]#同理
            print("the features_rgb was:\n")
            ...
            pseudo_labels_ir = cluster_ir.fit_predict(rerank_dist_ir)##labels
            ...
            pseudo_labels_rgb = cluster_rgb.fit_predict(rerank_dist_rgb)#question is 为标签的具体格式
            del rerank_dist_rgb
            del rerank_dist_ir
            num_cluster_ir = len(set(pseudo_labels_ir)) - (1 if -1 in pseudo_labels_ir else 0) #得到有效伪标签的具体数据
            num_cluster_rgb = len(set(pseudo_labels_rgb)) - (1 if -1 in pseudo_labels_rgb else 0)

        # generate new dataset and calculate cluster centers
        @torch.no_grad()
        def generate_cluster_features(labels, features):#这里的collection.defaultdict(list)这生成的是值为[]的字典
            centers = collections.defaultdict(list)
            for i, label in enumerate(labels):
                if label == -1:
                    continue
                centers[labels[i]].append(features[i])
                #这里的label作为key当然其的值为一个空列表，然后在使用append将值加入这个空列表
            centers = [
                torch.stack(centers[idx], dim=0).mean(0) for idx in sorted(centers.keys())
            ]#这里的stack对dim = 0 维度对形状相同的张量进行拼接,如何求平均，此处的center.keys()相当于标签

            centers = torch.stack(centers, dim=0)
            return centers

        cluster_features_ir = generate_cluster_features(pseudo_labels_ir, features_ir)##TODO:HERE ADD PART FEATURE
        ...
        memory_ir = ClusterMemory(model.module.num_features, num_cluster_ir, temp=args.temp,
                               momentum=args.momentum, use_hard=args.use_hard).cuda()##对memory bank操作
...
##############################
            ...
            mode='all'
            data_path="/home/shaorui/USL-VI-ReID-main/data/SYSU-MM01/"
...
def main_worker_stage2(args,log_s1_name,log_s2_name):
    ...
    # Create model
    model = create_model(args)
    checkpoint = load_checkpoint("/home/shaorui/USL-VI-ReID-main/data/logs/sysu_s2/checkpoint.pth.tar")
    model.load_state_dict(checkpoint['state_dict'])
    
    #Load model
    #checkpoint = torch.load("/home/shaorui/USL-VI-ReID-main/data/logs/sysu_s2/checkpoint.pth.tar")
